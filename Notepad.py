...
try:   #The "try" sequence checks if the relevant text file database is retrievable before running the rest of the program.
    ...
    Pad=open("notes.txt", "w")  #If a non-existing file is openned on "write" mode, the file will be created.
    Pad.close()
    
root=Tk()  #This line creates the main window for the graphics user interface (GUI)
root.title("Witpad")   #This is the name of the main window that appears.
...
root.config(bg="light green") #This is the colour of the window.
...
def Decryption(Ciphertext):  #The function can be called and accepts 1 input parameter which is the ciphertext to be decrypted
    Decrypt=[] #This sets up an empty array that the decrypted characters can be added to one by one.
    for cipher in Ciphertext:   #This is a Ceasar's Cipher algorithm with a shift of -1 and uses a count-controlled loop as it decrypts one chacracter at a time.
        ...
        Decrypt.append(plain)  #Adds the decrypted character to the array.
    Decrypt=str(Decrypt).replace("', '", "") #The these next 4 lines are used so the output string is in the correct format.
    Decrypt=Decrypt.replace("['", "") #The "replace" method is performed on a string and replaces the string in the first parameter present with the second parameter.
...
def Encryption(UserPwd):   #The function has 1 input parameter which is the plaintext that is to be encrypted
    ...
    for plain in UserPwd:  #"plain" is a local variable used here to refer to the plaintext character in an iteration of the count-controlled loop.
        cipher=chr(ord(plain)+1) #1 is added to the Unicode value of the character (this is the +1 shift of the Ceasar's Cipher)
...
#The "In" procedure is the main procedure that is called once a user successfully logs in.
def In(Account, line):   #Input parameters are Account(Decrypted) and the list position of the account when spilt by "~~~"
    def Logout():  #This nested procedure is attached to the "logout" button press event and causes the program to stop.
        exit()
    def Create():  #This nested procedure is attached to the "create new note" button press event.
        
        def enter(): #This nested procedure is attached to the "save" button press event, it saves the new note to the database.
            ...
            Pad=Pad1.read()  #The read method converts the contents of the text file into a string that can then be manipulated.
            Pad1.close()
            ...
            Pad=Pad.replace("\n", "")
            Pad=Pad.replace("]", "")
            Pad=Pad.replace("\\\\", "\\") #The character after a backslash is ignored by Python which is why I needed to put double the number of backslashes here as every other one is ignored.
            ...
            Pad1.write(Pad) #The "write" method re-writes the whole text file with the string passed to it.
            Pad1.close()  #The "close" method here ensures that the changes to the text file are saved.
            Enter.place_forget() #The "save" button widget is temporarily hidden from the window using the "place_forget" method.
            Pad=Pad.split("~~~")
            In(Decryption(Pad[line]), line) #Relaunches the "In" procedure incase the user wants to do more whilst logged in.
            
        Info.config(text="CLOSE ALL PREVIOUS NOTES!\nNEW NOTE:") #The Info label widget is edited so the text changes to "NEW NOTE" using the "config" method.
        ...
        Enter=Button(root, text="Save", height=3, width=6, command=enter) #This line creates the "save" Button widget and attaches it's button press event to call "enter" procedure above. 
        Enter.place(x=225, y=200)
        
    def Expand(Note, Num): #This procedure accepts 2 input parameters 1)the note 2)position of the note in the "Acc" array. (Look below). This displays each individual note.
        def Edit():  #This procedure is attached to the "Edit Note" button press event and allows the user to edit an old note without havig to delete it and create a new note.
            def Save(): #This save procedure replaces the old note with the edited version of it in the database. Works in the same way as the "enter" nested procedure above ^.
                Edit=Editor.get()
                ...
                Pad1.write(Pad)
                Pad1.close()
                save.destroy() #The following buttons are deleted (not just hidden) using the "destroy" method as they are no longer required.
                Inform.destroy()
                Editor.destroy()
                Note=Edit
            ...
            save.pack()
        def Delete():  #This nested procedure runs when the "Delete Note" button pressed event occurs and asks the user if they are sure they want to delete the note incase of accidental presses.
            def Yes():  #This procedure runs when the "Yes" button pressed event occurs and deletes the note permanently from the database. It works in a similar way to the "enter" procedure above,
                        #however this procedure removes a note rather than adding one.
                Pad1=open("notes.txt", "r", encoding="utf-8")
                Pad=Pad1.read()
                Pad1.close()
                Pad=Pad.split("~~~")
            ...
            def No(): #This nested procedure runs when the "No" button pressed event occurs and cancels stops the "Delete" procedure and takes the user back to the previous window layout
                Sure.destroy()
                yes.destroy()
                no.destroy()
                NOTE.pack()
            ...
            DelBut.pack_forget()
            EdBut.pack_forget()
            Sure=Label(Full, text="ARE YOU SURE YOU WANT TO DELETE THIS NOTE?\nMake sure to restart app after deleting a note :)", font="15") #This label widget is created which asks the user to confirm if they want to delete the note or not.
            Sure.pack()
            yes=Button(Full, text="YES", height=3, width=10, command=Yes) #These new buttons are created and placed on the window and execute different commands depending on what's pressed (look above for procedures).
            yes.pack()
            no=Button(Full, text="NO", height=3, width=10, command=No)
            no.pack()
        ...
        NOTE.pack()
        EdBut=Button(Full, text="Edit Note", height=3, width=10, command=Edit) #This edit button pressed event causes the "Edit" procedure to execute
        EdBut.pack()
        DelBut=Button(Full, text="Delete Note", height=3, width=10, command=Delete) #This delete button pressed event causes the "Delete" procedure to execute
        DelBut.pack()
        
    def Search(): #NEWLY ADDED SEARCH FEATURE
        SearchBar= Entry(root, width=75) #This widget is the place where the user enters the search term
        SearchBar.place(x=40, y=100)
        Info.config(text="Enter search term below:")
        def Enter5():
            Term=SearchBar.get()
            pos=0
            for note in Acc:
                pos+=1
                Info1.place(x=50, y=200)
                if Term in note: #Checks if their search term is present in any note
                    Info2.place(x=50, y=220)
                    Info1.config(text=f"Term Found in Note {pos}") #Displays note number and actual note on main window if term found
                    Info2.config(text=note)
                    break
                else:
                    Info1.config(text="Term Not Found :(") #Displays this if search term not located in any of the notes
                    Info2.place_forget()
                def Contin():
                    Enter.destroy()
                    SearchBar.destroy()
                    ...
                    Info1.place_forget()
                    Info2.place_forget()
                    Info.config(text="Sticky Notes:")
                Continue=Button(root, text="End Search", command=Contin)
                Continue.place(x=50, y=250)
        Enter= Button(root, text="Enter", command=Enter5)
    ...
    Info1.place_forget()
    Info2.place_forget()
    CreateBut=Button(root, text="Create New Note", height=5, width=15, command=Create) #When this button is pressed, the "Create" procedure above executues
    CreateBut.place(x=350, y=350)
    Info.config(text="Sticky Notes:", bg="light green", font="ArielBold 12")
    Info.place(x=225, y=50)
    SearchBut= Button(root, text="Search Notes", height=5, width=15, command=Search) #Adding in a search feature to allow users to find parts of their note quickly
    SearchBut.place(x=40, y=350)
    logout=Button(root, text="Exit", height=5, width=15, activebackground="red", command=Logout)
    ...
    if len(Account) < 3:  #This checks if the user has any notes in their account at all by checking if the length of the array is >3 or not as notes come after Username and PWD.
        ...
        Odd=0
        for i in Acc:
            i=Button(root, text=Acc[Odd], width=30, command=Expand(Acc[Odd], Odd+1)) #Each element (Note) is run through the expand procedure above which diplays them on separate windows.
            Odd=Odd+1

#This Signup procedure is attched to the "Register" button pressed event and takes the user through the process of creating a new account and then logging them in automatically.
def Signup():
    Main.place(x=150, y=0)
    def enter(): #This procedure runs when the "Enter" button pressed event occurs and runs checks on the data provided by the user before creating them a new account.
        Pad1=open("notes.txt", "r", encoding="utf-8")
        Pad=Pad1.read()
        Pad1.close()
        ...
        if len(PWD.get()) < 8:  #This length check ensures that 1)The entery box hasn't been left blank; 2)The password is of a decent strength (8 characyers long at least)
            ...
            Pad1.close()
            Enter.destroy()
            In(Decryption(Pad[-1]), len(Pad)-1) #This runs the "In" procedure and passes the account and position of account in the database to that procedure.
...
#This "Login" procedure is attached to the "Login" button pressed event and asks the user to input thir username and password to check if their account exists and may log them in.
def Login():
    ...
    def enter():
        ...
        for line in Pad: #line is one of the elements in Pad so is an individual account.
            ...
            if Encrypt == line: #It then compares the encrypted version of the username and password the user has enetered to see if it matches any values of "line" (look above)
                ...
                Main.place(x=200, y=0)
                Enter.destroy()
                In(Decryption(Pad[nom]), nom) #The "Login" procedure now calls the "In" functtion to take over from here and gives it the account and account position in the database.
...
#All the widgets that are used in more than one procedure are created here so they are global variables.
...
#Read the code top down and all methods are explained upon first usage.
#Emphasize on: encryption (solves security issue), front end GUI, backend database, error handling, procedures, nested procedures, data sanitisation (including checks), a realistic interface (UX)
#Security Features: Encryption, Only the individual's account is decrypted when database is retrieved, Password appears as "*", Passwords need to be atleast 8 characters long.
#Explain how the database was structured and why so many ".replace" lines were required.
#Explain the concept of global and local variables; procedures and variable passing; events
#Make a success criteria
#Explain the basic core concept of the code in English.
#Say why I used for loops instead of while loops so they know I can use it and give an example of what it may have looked like (eg from the encryption procedure)
#What tkinter is and why I used it.
#Define every technical term used for communication marks.
#LEARNT: When an array in Python is given another name, the same array can be referred to by 2 names and changes in one name thus change the array in the other name so I had to use the .copy method.
